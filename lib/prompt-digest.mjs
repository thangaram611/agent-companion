// Smart-transcript digest for an ACP prompt (Copilot, Antigravity).
//
// Reads the private prompt JSONL stream and emits a markdown summary the
...
// supervisor alert (~60s cadence during silence), and on terminal.
...
import { digestPathForJob, promptEventsPath, writePrivateFileAtomic } from './runtime-paths.mjs';
import { formatUsage } from './usage.mjs';
...
export function digestPath(jobId) {
  ...
  try { return digestPathForJob(jobId); }
  catch { return null; }
}
...
export function buildDigest(promptId, jobMeta = {}) {
  const events = readPromptEvents(promptId, jobMeta.target);
...
export function writeDigest(promptId, jobMeta = {}) {
  ...
  if (!path) return null;
  try {
    // Atomic (temp + rename), not truncate-then-write. The bridge regenerates
    // this file on every status request and every supervisor alert while other
    // processes read the same path; a truncating write hands them a zero-length
    // or half-written digest (measured: 20,945 of 119,401 concurrent
    // cross-process reads returned 0 bytes). `digestPath` has already created
    // the digest dir via `digestDir()`, and the temp file lands in that same dir
    // so the rename stays intra-filesystem. Mode is 0600 either way.
...
export function renderDigest(events, promptId, jobMeta = {}) {
  const {
    ...
    parallel, task, sessionId, verdict, verdictReason, usage,
    // The companion's display name; Copilot when the caller named none, so
    // every pre-existing digest renders exactly as it did.
    targetLabel = 'Copilot',
  } = jobMeta;

  const lines = [];
  lines.push(`# ${targetLabel} job ${jobId || '(unknown)'} — digest`);
  lines.push('');
  lines.push(`**Updated:** ${new Date().toISOString()}`);
  if (status)    lines.push(`**Status:** \`${status}\``);
  if (mode)      lines.push(`**Mode:** ${mode}${parallel ? ' (parallel via /fleet)' : ''}`);
  if (template)  lines.push(`**Template:** ${template}`);
  // The job's usage, the same line the target-neutral writer renders.
  if (usage)     lines.push(`**Usage:** ${formatUsage(usage)}`);
  // A completed `review` job's parsed verdict — same line the target-neutral
  // writer in bridge-server/opencode-runtime.mjs renders.
  if (verdict !== undefined) lines.push(`**Verdict:** ${verdict ?? `none (${verdictReason || 'unknown'})`}`);
...
function readPromptEvents(promptId, companion) {
  if (!promptId) return null;
  let path;
  try { path = companion ? promptEventsPath(promptId, companion) : promptEventsPath(promptId); }
  catch { return null; }
