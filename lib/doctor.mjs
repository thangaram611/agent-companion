...
import { acpDaemonSnapshot } from '../bridge-server/daemon-client.mjs';
import { listTargets } from './target-registry.mjs';
...
export function buildDoctorReport({
  ...
  const report = {
    ...
    runtime: {
      ...
      digestDir: digestDir(),
      // One daemon per ACP companion: its socket, log and the shared-runtime
      // registry entry a bridge recorded for it (null when none has run).
      acpDaemons: Object.fromEntries(listTargets(env)
        .filter((t) => t.capabilities?.acp)
        .map((t) => [t.id, { socket: daemonSocketPath(t.id), log: daemonLogFile(t.id), registry: acpDaemonSnapshot(t.id) }])),
      codexAppServer,
    },
  };
  ...
  if (codexAppServer.active) {
    ...
    if (install.installed?.quarantined) {
      warnings.push(`The installed Codex binary is quarantined (${install.installed.path}); doctor made no xattr changes. Strip it yourself with \`xattr -d com.apple.quarantine\` on the binary and its codex-code-mode-host helper, and again after each cask upgrade — Homebrew dropped --no-quarantine (brew#20755), so no install-time switch exists.`);
