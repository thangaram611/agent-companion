...
// notification, which is most of them (0.150.1 → 0.152.1 added two), and says
...
// Measured on codex-cli 0.147.0 and re-measured on 0.152.1.
// ---------------------------------------------------------------------------
...
// (read at rust-v0.152.1: mcp_event_stream.rs, command_exec.rs,
...
const THREAD_ITEM_VARIANTS = [
  ...
  'enteredReviewMode', 'exitedReviewMode', 'fileChange', 'functionCallOutput', 'hookPrompt',
  'imageGeneration', 'imageView', 'mcpToolCall', 'plan', 'reasoning', 'sleep', 'subAgentActivity',
...
test('model-provider auth recovery notifications stay with their owning thread', () => {
  const params = {
  ...
  for (const method of [
    ...
    assert.deepEqual(contract.serverNotifications.find((entry) => entry.method === method), {
      method,
      routing: 'threadId',
      key: 'params.threadId',
      optional: false,
      params: {
        message: 'string', provider: 'string', threadId: 'string', turnId: 'string',
      },
    });
    assert.deepEqual(routeNotification(method, params), {
      routing: 'threadId', threadId: 't-auth', owner: null, optional: false,
    });
    assert.equal(serverFrameViolation(method, params), null);
    assert.match(serverFrameViolation(method, { ...params, threadId: undefined }), /missing required field `threadId`/);
  }
});
...
test('the error notification carries a TurnError, a turn id and willRetry', () => {
  ...
  // New in 0.152.1 and optional: the adapter's existing message read remains
  // authoritative, while richer clients may inspect the recovery guidance.
  assert.equal(contract.structuredTypes.TurnError.misalignment, 'MisalignmentErrorDetails?');
});
...
test('a Turn carries its status, its items and a TurnError, and no `failure`', () => {
  ...
  // `historyMode` became part of the stable Thread response in 0.152.1. It is
  // defaulted/optional on the wire, so old fixtures and current readers remain
  // valid while paginated clients can select the new list methods.
  assert.equal(contract.structuredTypes.Thread.historyMode, 'enum:legacy|paginated?');
});
...
test('the ThreadItem union is the pinned variant list, and `error` is not one of them', () => {
  ...
  // This is the model-visible OUTPUT supplied back for an unpaired/named
  // function call, not another invocation. Invocation items remain the
  // commandExecution/dynamicToolCall/mcpToolCall variants, so an accumulator
  // must tolerate this without inflating its executed-tool count.
  assert.deepEqual(contract.threadItems.functionCallOutput, {
    id: 'string', name: 'string', namespace: 'string?', output: 'any', type: 'enum:functionCallOutput',
  });
  assert.equal(threadItemViolation({
    type: 'functionCallOutput', id: 'f1', name: 'shell', output: { content: 'ok' },
  }), null);
  assert.match(
    threadItemViolation({ type: 'functionCallOutput', id: 'f1', name: 'shell' }),
...
test('the structured types are the required-field closure, and nothing wider', () => {
  ...
  for (const optionalOnly of ['MemoryCitation', 'McpToolCallResult', 'GitInfo', 'MisalignmentErrorDetails', 'WebSearchAction']) {
...
test('the params object itself is required, per method', () => {
  ...
  // params type has a required field of its own — all but eight requests list
...
test('the stable paginated-history requests publish their required anchors', () => {
  assert.deepEqual(clientRequestContract('thread/turns/list'), {
    method: 'thread/turns/list', paramsRequired: true, required: ['threadId'],
  });
  assert.deepEqual(clientRequestContract('thread/items/list'), {
    method: 'thread/items/list', paramsRequired: true, required: ['threadId'],
  });
  assert.deepEqual(clientRequestContract('thread/revert'), {
    method: 'thread/revert', paramsRequired: true, required: ['beforeTurnId', 'threadId'],
  });
  // Reading or mutating an existing thread is not ownership. Only start,
  // resume and fork make the broker attach a connection to a returned thread.
  for (const method of ['thread/turns/list', 'thread/items/list', 'thread/revert']) {
    assert.equal(THREAD_OWNERSHIP_METHODS.has(method), false, method);
  }
});
...
test('a method no fake can answer is refused — without reciting an inventory the fixture lacks', () => {
  ...
  // The measurement that decides the wording. The Rust ClientRequest union
  // still contains experimental methods omitted by the default schema dump;
  // `turn/settings/update` is explicitly experimental at rust-v0.152.1 and is
  // absent here. A fake therefore cannot recite this fixture as the server's
  // complete inventory, even though the three now-stable history methods are
  // present.
  assert.equal(clientRequestContract('turn/settings/update'), null);
