...
const GLOBAL_METHODS = [
  ...
  // Global by the distiller's rule (no thread id in params), but codex delivers
  // it only to the connection that owns `params.subscriptionId` — so through a
  // shared broker it would be broadcast to every bridge. Dormant: nothing here
  // calls `mcpServer/event/stream/start`. The adapter that first does must
  // teach the broker to route by subscription before it does.
...
test('the pinned contract routes every notification the way the broker was built against', () => {
  ...
  // a fourth routing class cannot appear without being named here.
  const classified = new Set([...GLOBAL_METHODS, ...NESTED_METHODS]);
