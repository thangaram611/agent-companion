...
test('configuredDefault reflects the env default-target override', () => {
  ...
  assert.deepEqual(selectConfiguredTarget({ env }), { target: 'copilot', source: 'env' });
});

test('selectConfiguredTarget reports unset with no config', () => {
  assert.deepEqual(selectConfiguredTarget({ env: {} }), { target: null, source: 'unset' });
});
