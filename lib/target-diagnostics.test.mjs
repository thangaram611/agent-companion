...
//   antigravity: false | true | { keychain: boolean }  (default keychain: true)
function makeRun({ opencode = false, copilot = false, codex = false, antigravity = false } = {}) {
  return (cmd, args = []) => {
    const a = args.join(' ');
    const isOpenCode = /opencode/.test(cmd);
    const isCopilot = /copilot/.test(cmd);
    const isCodex = /codex/.test(cmd);
    const isAntigravity = /agy_acp_server/.test(cmd);
    if (isAntigravity) {
      if (!antigravity) return { ok: false, output: 'command not found' };
      // The registry binary prints a build stamp, not a semver (1.1.1, measured).
      if (a === '--version') return { ok: true, output: 'Built on Thu Sep  3 00:22:52 2026 (1788375172)\nBuilt by …' };
      return { ok: false, output: 'unknown flag' };
    }
    if (cmd === 'security') {
      // `find-generic-password` without `-w`: exit 0 and the item's attributes
      // when present, exit 44 when not — never the secret.
      const present = antigravity && antigravity.keychain !== false && a === 'find-generic-password -s gemini -a antigravity-acp';
      return present ? { ok: true, output: 'keychain: "login.keychain-db"\n    "acct"<blob>="antigravity-acp"' } : { ok: false, output: 'The specified item could not be found in the keychain.' };
    }
    if (isOpenCode) {
...
test('inspectTargets covers every registered target', () => {
  const run = makeRun({ opencode: { models: 'anthropic/x' }, copilot: true, codex: true, antigravity: true });
  const settings = JSON.stringify({ auth: { type: 'oauth-personal' } });
  const all = inspectTargets({ run, env: {}, readFile: (path) => (path.endsWith('/antigravity-acp/settings.json') ? settings : null), home: '/home/u', platform: 'darwin' });
  assert.deepEqual(Object.keys(all).sort(), ['antigravity', 'codex', 'copilot', 'opencode']);
  assert.equal(all.opencode.ready, true);
  assert.equal(all.copilot.ready, true);
  assert.equal(all.codex.ready, true);
  assert.equal(all.antigravity.ready, true);
});

// --------------------------------------------------------- antigravity

// The auth probe is a credential read, never a turn: the server's own
// `settings.json` (`auth.type`) through the injected reader, the keychain item
// through the injected runner — both measured 2026-09-11 on agy_acp_server 1.1.1.
test('antigravity: the auth probe reads the server\'s own credential state without a turn, and says which', () => {
  const home = '/home/u';
  const files = (map) => (path) => map[path] ?? null;
  const settings = (type) => JSON.stringify({ auth: { type } });
  const opts = (extra) => ({ run: makeRun({ antigravity: true }), env: {}, readFile: files({}), home, platform: 'darwin', ...extra });

  let r = inspectTarget('antigravity', opts({ readFile: files({ '/home/u/.gemini/antigravity-acp/settings.json': settings('oauth-personal') }) }));
  assert.equal(r.installed, true);
  assert.equal(r.version, 'Built on Thu Sep  3 00:22:52 2026 (1788375172)');
  assert.equal(r.authenticated, true, 'auth.type recorded and the keychain item present');
  assert.equal(r.ready, true);
  assert.deepEqual(r.blockers, []);

  r = inspectTarget('antigravity', opts({ run: makeRun({ antigravity: { keychain: false } }), readFile: files({ '/home/u/.gemini/antigravity-acp/settings.json': settings('oauth-personal') }) }));
  assert.equal(r.authenticated, false, 'the selection without its token does not count');
  assert.equal(r.blockers[0].code, 'needs_auth');
  assert.match(r.blockers[0].message, /no keychain item/);

  r = inspectTarget('antigravity', opts({}));
  assert.equal(r.authenticated, false);
  assert.match(r.blockers[0].message, /has not been signed in/);
  assert.ok(r.nextSteps.some((step) => /install-antigravity-acp\.mjs --login/.test(step)), 'the next step is the login the install script drives');

  r = inspectTarget('antigravity', opts({ env: { GEMINI_API_KEY: 'k' }, readFile: files({ '/home/u/.gemini/antigravity-acp/settings.json': settings('gemini-api-key') }) }));
  assert.equal(r.authenticated, true, 'an API-key selection with the key set counts');
  r = inspectTarget('antigravity', opts({ readFile: files({ '/home/u/.gemini/antigravity-acp/settings.json': settings('gemini-api-key') }) }));
  assert.equal(r.authenticated, false, 'and without the key it does not');

  // Elsewhere the token store is unmeasured: unknown, never guessed.
  r = inspectTarget('antigravity', opts({ platform: 'linux', readFile: files({ '/home/u/.gemini/antigravity-acp/settings.json': settings('oauth-personal') }) }));
  assert.equal(r.authenticated, 'unknown');
  assert.equal(r.ready, true, 'unknown auth is a warning, as for Copilot');

  // GEMINI_HOME relocates the server's home, as the server itself honours it.
  r = inspectTarget('antigravity', opts({ env: { GEMINI_HOME: '/elsewhere' }, readFile: files({ '/elsewhere/antigravity-acp/settings.json': settings('oauth-personal') }) }));
  assert.equal(r.authenticated, true);

  // Not installed: nothing is probed and the install command is the blocker.
  r = inspectTarget('antigravity', opts({ run: makeRun({ antigravity: false }) }));
  assert.equal(r.installed, false);
  assert.equal(r.authenticated, 'unknown');
  assert.equal(r.blockers[0].code, 'missing_binary');
  assert.match(r.blockers[0].message, /install-antigravity-acp\.mjs/);
});

test('antigravity: the permission policy is a closed set with `all` as the default, and nothing in it is flagged dangerous', () => {
  const run = makeRun({ antigravity: true });
  const readFile = () => null;
  let r = inspectTarget('antigravity', { run, env: {}, readFile, home: '/home/u', platform: 'darwin' });
  assert.deepEqual(r.permission, { mode: 'all', source: 'default', readyForNonInteractive: true, risk: 'normal' });
  r = inspectTarget('antigravity', { run, env: { AGENT_COMPANION_ANTIGRAVITY_PERMISSION: 'edit' }, readFile, home: '/home/u', platform: 'darwin' });
  assert.deepEqual(r.permission, { mode: 'edit', source: 'env', readyForNonInteractive: true, risk: 'normal' });
  r = inspectTarget('antigravity', { run, env: { AGENT_COMPANION_ANTIGRAVITY_PERMISSION: 'yolo' }, readFile, home: '/home/u', platform: 'darwin' });
  assert.deepEqual(r.permission, { mode: 'all', source: 'fallback', readyForNonInteractive: true, risk: 'normal' }, 'the server\'s own mode names are not this knob');
  assert.deepEqual(r.warnings.filter((w) => w.code !== 'auth_unknown' && w.code !== 'needs_auth').map((w) => w.code), []);
});
...
test('inspectProfile: adapter coherence covers EVERY companion\'s daemon adapter', () => {
  // The check used to compare against the bare string 'server', which is
  // opencode's vocabulary — so a codex profile asking for `appserver` on a
  // machine where the app-server is unavailable was reported ready. It reads the
  // registry's daemon-adapter table now, and the two companions are asserted
  // together because the bug was the asymmetry, not either case alone.
  //
  // `load` is injected rather than written to disk: loadProfiles OVERLAYS the
  // adapter env when it resolves capabilities, so a profile read back off disk
  // always has serverMode true and could never exercise this branch. The
  // incoherent pair — daemon adapter declared, daemon capability absent — is what
  // a doctor run on a machine missing the transport is meant to catch.
  const incoherent = (id, companion, adapter) => ({ id, companion, adapter, model: null, strengths: [], capabilities: { send: true, serverMode: false } });
  const load = { byId: new Map([
    ['cx-app', incoherent('cx-app', 'codex', 'appserver')],
    ['oc-srv', incoherent('oc-srv', 'opencode', 'server')],
    ['cx-exec', incoherent('cx-exec', 'codex', 'exec')],
  ]) };
  const run = makeRun({ codex: true, opencode: { models: '' } });
  for (const id of ['cx-app', 'oc-srv']) {
    const r = inspectProfile(id, { run, env: {}, load });
    assert.equal(r.adapterCoherent, false, id);
    assert.equal(r.ready, false, id);
    assert.ok(r.blockers.some((b) => b.code === 'adapter_unavailable'), id);
    // The message names the adapter that was actually asked for, not opencode's.
    assert.ok(r.blockers.some((b) => b.message.includes(`adapter:"${load.byId.get(id).adapter}"`)), id);
  }
  // The single-shot adapter asks for FEWER capabilities, so it is always coherent.
  const exec = inspectProfile('cx-exec', { run, env: {}, load });
  assert.equal(exec.adapterCoherent, true);
  assert.ok(!exec.blockers.some((b) => b.code === 'adapter_unavailable'));
});
