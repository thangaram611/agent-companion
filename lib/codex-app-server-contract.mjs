...
// The three responses that carry a `result.thread.id` the CALLER now owns: it
// asked for that thread, so it is by construction the client that wants its
// events and the client entitled to act on it. Both consumers key off this one
// fact for their own purpose — the broker subscribes the asking client to the
// thread (with no extra round trip), the adapter marks the connection attached
// so `turn/interrupt` and `turn/steer` may skip their resume-first guard — and a
// codex that grew a fourth such method with only one of the two updated would
// leave a bridge acting on a thread the broker never routed events for.
