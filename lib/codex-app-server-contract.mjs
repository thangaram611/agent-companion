...
// Three refusals measured on 0.147.0 that this does NOT reproduce, all the same
...
//                                                    expected a string
// Closing them means recording the discriminator of a nested tagged union
// (`UserInput`) and per-field scalar types, neither of which the fixture keeps.
// Latent as of this contract: every call site builds `input` from the adapter's
// own `{type:'text', text}` literal and every id it sends is already a string.
// A checker that guessed at those shapes would be the hand-written table this
// module exists to avoid, so the gap is recorded here instead of half-closed —
