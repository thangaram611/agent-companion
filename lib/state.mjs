...
//   default-target                    — optional one-line target adapter id
...
export const DEFAULT_TARGET = 'opencode';
...
// ------------------------------------------------------ default-target
//
// Single-line file, or AGENT_COMPANION_DEFAULT_TARGET env override. Empty or
// missing uses the current bootstrap fallback until target-management UX exists.

export function readDefaultTarget(env = process.env) {
  const envTarget = String(env.AGENT_COMPANION_DEFAULT_TARGET || '').trim();
  if (envTarget) return { target: envTarget, source: 'env' };
  const legacyEnvTarget = String(env.COPILOT_COMPANION_DEFAULT_TARGET || '').trim();
  if (legacyEnvTarget) return { target: legacyEnvTarget, source: 'legacy-env' };
  const raw = readFileSafe(TARGET_FILE);
  if (raw == null) return { target: DEFAULT_TARGET, source: 'fallback' };
  const target = raw.trim();
  if (!target) return { target: DEFAULT_TARGET, source: 'fallback' };
  return { target, source: 'config' };
}
