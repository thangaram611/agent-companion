...
// The predicate, owned here for the same reason the walk is. Both daemons read
// AND sweep the same directory: if one of them tuned its own copy of these down,
// its next idle tick would unlink heartbeats the other still counts as live, and
// that daemon would then reap itself out from under an active host session.
//
// HOST_LIVENESS_TTL_MS is the "is a host still working?" window and must stay in
// step with the hook cadence in hooks/drain-completions.sh.
// HEARTBEAT_STALE_AFTER_MS is the "this file is litter" window (parent crash, OS
// reboot). The helper's invariant is liveTtlMs < staleAfterMs; these satisfy it.
export const HOST_LIVENESS_TTL_MS = 30 * 60 * 1000; // 30 min
export const HEARTBEAT_STALE_AFTER_MS = 24 * 60 * 60 * 1000; // 24h
