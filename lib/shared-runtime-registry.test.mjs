...
// --- the race the confirmation exists for ----------------------------------
//
// Two bridges over one registry file: one disposing, one adopting, with the
// adoption landing INSIDE the dispose. `claimDisposal`'s own confirm cannot
// catch that one — it ran before the dispose began — and the adopter has
// nothing else to warn with: a lease needs a job, and adoption comes first. The
// clock and the pid probe are injected, because a test that waited on either
// would be asserting about this machine rather than about the protocol.

test('an adopter that lands INSIDE the dispose window makes the disposer stand down', async () => {
  _setForTest({ pidAlive: () => true });
  const now = 60_000;
  const decided = { endpoint: 'unix:/a', lastUsedAt: 1 };
  seed(decided);

  const adopter = makeRegistry();
  const acted = [];
  let verdict = null;
  const disposer = makeRegistry({
    dispose: async (entry, { confirmDisposal }) => {
      // The other bridge probed the runtime, found it healthy and recorded its
      // adoption — merging into the entry, so our claim is still on it. All it
      // has changed is `lastUsedAt`.
      adopter.record({ ...adopter.read(), endpoint: 'unix:/a' }, { now: now + 10 });
      verdict = confirmDisposal({ now: now + 20 });
      if (verdict) acted.push(entry.endpoint);
    },
  });

  const claimed = await disposer.reapIdle({ idleMs: 1_000, now, pid: 111 });
  assert.equal(claimed, true, 'the claim itself was taken — the stand-down is later');
  assert.equal(verdict, false, 'the disposer must be told to stand down');
  assert.deepEqual(acted, [], 'and must not act');
  assert.equal(onDisk().disposing, undefined, 'a leftover claim would block the adopter until the TTL expired');
  assert.equal(onDisk().lastUsedAt, now + 10, 'the adopter\'s entry must survive the reap that stood down');
});

test('a lease taken INSIDE the dispose window stops the disposer too', async () => {
  // The adopter got further: its job started and stamped a lease. Same answer,
  // via the other half of the same check.
  _setForTest({ pidAlive: () => true });
  const now = 60_000;
  seed({ endpoint: 'unix:/a', lastUsedAt: 1 });
  ...
  const disposer = makeRegistry({
    dispose: async (entry, { confirmDisposal }) => {
      seed({ ...onDisk(), leases: lease(7, 'theirs', now + 10) });
      if (confirmDisposal({ now: now + 20 })) acted.push(entry.endpoint);
    },
  });

  await disposer.reapIdle({ idleMs: 1_000, now, pid: 111 });
  assert.deepEqual(acted, []);
  assert.ok(onDisk().leases['7:theirs'], 'and the lease is not erased by the reap that stood down');
});

test('nobody got in, so the disposer confirms and acts', async () => {
  // The other side of the same check: a confirmation that refused whenever it
  // was asked would be indistinguishable from no reaper at all.
  _setForTest({ pidAlive: () => true });
  const now = 60_000;
  seed({ endpoint: 'unix:/a', lastUsedAt: 1 });
  ...
  const disposer = makeRegistry({
    dispose: async (entry, { confirmDisposal }) => {
      if (confirmDisposal()) acted.push(entry.endpoint);
    },
  });

  assert.equal(await disposer.reapIdle({ idleMs: 1_000, now, pid: 111 }), true);
  assert.deepEqual(acted, ['unix:/a']);
  assert.equal(disposer.read(), null, 'and the entry is retired');
});

test('an on-demand disposer can run the same two-phase protocol off the public surface', async () => {
  // `claimDisposal` is exported for an adapter that disposes on demand rather
  // than on idleness. That adapter has the same gap `reapIdle` has — it drains,
  // or asks, between claiming and acting — so the second half of the protocol
  // has to be reachable too. Handing out the claim without the confirmation
  // would ship exactly the bug this module was fixed for.
  _setForTest({ pidAlive: () => true });
  const now = 60_000;
  const decided = { endpoint: 'unix:/a', lastUsedAt: 1 };
  seed(decided);
  ...
  const reg = makeRegistry();
  assert.equal(typeof reg.confirmDisposal, 'function', 'the claimer needs the check it is told to call');
  assert.equal(reg.claimDisposal(decided, { now, pid: 111 }), true);

  // ...it drains for a while, and a bridge adopts the runtime meanwhile.
  makeRegistry().record({ ...decided }, { now: now + 10 });

  assert.equal(reg.confirmDisposal(decided, { now: now + 20, pid: 111 }), false, 'and must be told to stand down');
  assert.equal(onDisk().disposing, undefined, 'standing down withdraws the claim');
  assert.equal(onDisk().lastUsedAt, now + 10, 'the adopter keeps the runtime');
});
