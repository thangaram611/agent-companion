...
test('default-target config falls back, honors env overrides, round-trips, and writes atomically', () => {
  state.clearDefaultTarget();
  assert.deepEqual(state.readDefaultTarget({}), { target: state.DEFAULT_TARGET, source: 'fallback' });
  assert.deepEqual(
    state.readDefaultTarget({ AGENT_COMPANION_DEFAULT_TARGET: 'copilot' }),
    { target: 'copilot', source: 'env' },
  );
  assert.deepEqual(
    state.readDefaultTarget({ COPILOT_COMPANION_DEFAULT_TARGET: 'copilot' }),
    { target: 'copilot', source: 'legacy-env' },
  );
