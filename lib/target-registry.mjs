...
const TARGETS = {
  opencode: {
    ...
    notes: [
      ...
      'Usage: server mode reads the assistant message info (`tokens`, `cost` in USD, `modelID`) live and from the transcript on resume; the CLI adapter reads `step-finish` parts off the JSON stream when present — schema-derived from OpenCode\'s OpenAPI document, not live-measured.',
    ],
  ...
  copilot: {
    ...
    notes: [
      ...
      'Usage: the ACP stream carries none (measured, Copilot CLI 1.0.77); the daemon reads the prompt\'s `invoke_agent` span off the OTEL file exporter it enables (`COPILOT_OTEL_FILE_EXPORTER_PATH`) — tokens, cache read/write, reasoning, model and `github.copilot.cost` in premium requests — and attaches it to the completed prompt. A prompt that did not complete carries none.',
    ],
