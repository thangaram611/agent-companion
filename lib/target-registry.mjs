...
export const TARGET_IDS = new Set(['opencode', 'copilot', 'codex', 'antigravity']);

// --- Antigravity helpers -------------------------------------------------------
//
// The daemon spawns the ACP registry's `agy_acp_server.par`, NOT the `agy` CLI:
// `agy` 1.2.0 has no ACP mode (its headless surface is `-p`; feature request
// google-antigravity/antigravity-cli#31 is still open), and the `.par` is the
// binary Google publishes for ACP clients — registry entry `antigravity-acp`,
// the one Zed, JetBrains and Xcode launch on the same personal login. There is
// no brew formula or npm package for it, so `scripts/install-antigravity-acp.mjs`
// puts the registry's current version here; `ANTIGRAVITY_ACP_BIN` overrides.
export function antigravityAcpPaths(env = process.env, home = homedir()) {
  const dataHome = String(env.XDG_DATA_HOME || '').trim() || join(home, '.local', 'share');
  const root = join(dataHome, 'agent-companion', 'antigravity-acp');
  return { root, current: join(root, 'current'), binary: join(root, 'current', 'agy_acp_server.par') };
}

// The daemon's answer to `session/request_permission`: allow everything, allow
// tool calls of kind `edit` only, or allow nothing that asks. The server
// itself asks in its `default` session mode for `execute` and `edit` tool
// calls and never for reads (measured 2026-09-11 on 1.1.1); it also offers a
// `yolo` session mode, which the daemon does not set — the answer policy is
// what keeps an unattended run unattended, and it is the daemon's to give.
const ANTIGRAVITY_PERMISSION_POLICIES = ['all', 'edit', 'none'];
export function resolveAntigravityPermission(env = process.env) {
  const raw = String(env.AGENT_COMPANION_ANTIGRAVITY_PERMISSION || '').trim().toLowerCase();
  return ANTIGRAVITY_PERMISSION_POLICIES.includes(raw) ? raw : 'all';
}

// The ACP server's own credential state, read without a turn. Measured
// 2026-09-11 on agy_acp_server 1.1.1: the login records `auth.type` in
// `$GEMINI_HOME/antigravity-acp/settings.json` (`~/.gemini` by default; the
// server logs which) and keeps the OAuth token in the login keychain as
// service `gemini`, account `antigravity-acp` — it shares nothing with `agy`
// or with Gemini CLI's `oauth_creds.json`. `security find-generic-password`
// without `-w` reports the item's presence and never the secret; elsewhere the
// token store is unmeasured and the verdict stays `unknown` rather than guessed.
export function probeAntigravityAuth({ env, home, readFile, run, platform = process.platform }) {
  const geminiHome = String(env.GEMINI_HOME || '').trim() || join(home, '.gemini');
  const settingsPath = join(geminiHome, 'antigravity-acp', 'settings.json');
  let type = null;
  try { type = JSON.parse(readFile(settingsPath) || 'null')?.auth?.type ?? null; } catch { type = null; }
  if (!type) {
    return { authenticated: false, detail: `no auth.type in ${settingsPath} — the ACP server has not been signed in on this machine` };
  }
  if (type === 'gemini-api-key') {
    return env.GEMINI_API_KEY
      ? { authenticated: true, detail: 'gemini-api-key (GEMINI_API_KEY)' }
      : { authenticated: false, detail: 'auth.type is gemini-api-key but GEMINI_API_KEY is not set' };
  }
  if (type === 'oauth-personal') {
    if (platform !== 'darwin') {
      return { authenticated: 'unknown', detail: `auth.type oauth-personal recorded; the token store is not probed on ${platform}` };
    }
    const res = run('security', ['find-generic-password', '-s', 'gemini', '-a', 'antigravity-acp']);
    return res.ok
      ? { authenticated: true, detail: 'oauth-personal (keychain item present)' }
      : { authenticated: false, detail: 'auth.type oauth-personal recorded but no keychain item (service gemini, account antigravity-acp) — sign in again' };
  }
  return { authenticated: 'unknown', detail: `auth.type ${type} recorded; that method is not probed` };
}
...
const TARGETS = {
  ...
  antigravity: {
    ...
    capabilities: {
      ...
      status: true,
      cancel: true,
      // Daemon properties, as for Copilot: reply is cancel + re-prompt on the
      // same ACP session, resume is the detached daemon outliving the bridge
      // plus `session/load` for a session the daemon no longer holds.
      reply: true,
      resume: true,
      jsonEvents: true,
      acp: true,
      serverMode: false,
      parallel: 'planned',
      // A model is a per-session config option on this server (see `acp.setModel`).
      modelSelection: true,
    },
    notes: [
      'Adapter drives the generic ACP daemon (`scripts/acp-daemon.mjs --companion antigravity`): one detached daemon per host home owning one `agy_acp_server.par` child over stdio, with its own socket, log and prompt streams beside Copilot\'s. The child is Google\'s own ACP server from the ACP registry (`antigravity-acp`, the binary Zed, JetBrains and Xcode launch), never the `agy` CLI, which has no ACP mode (1.2.0; antigravity-cli#31 open). Install it with `node scripts/install-antigravity-acp.mjs`, sign it in with `--login`.',
      'Terms: the bridge is a stdio client of Google\'s signed server, like the editors Google documents; it never holds the OAuth token and never calls Google\'s backend — the conduct Google\'s terms and FAQ prohibit ("third party software, tools, or services to access the Service (e.g. using OpenClaw with Antigravity OAuth)"). docs/MVP_TRACKER.md item 8 records the reading and its caveats; the account risk is the operator\'s.',
      'Measured 2026-09-11 on agy_acp_server 1.1.1: answers protocolVersion 1 when asked 1 (and 2 when asked 2, so the pin holds); advertises loadSession and session list/resume; a session survives the server process and `session/load` replays its history and remembers, so this descriptor honours it. The default session mode asks `session/request_permission` for `execute` and `edit` tool calls (options allow_always / allow_once / reject_once; edits get no allow_always) and never for reads; the daemon answers per `AGENT_COMPANION_ANTIGRAVITY_PERMISSION=all|edit|none` (default `all`). `session/cancel` mid-turn answers `stopReason: cancelled`. The turn\'s model is a session config option (`session/set_config_option`, id `model`, ids like `gemini-3.7-flash-high`, `gemini-pro-agent`; an unknown id is -32602 naming the available ones) and is NOT kept by `session/load`, so the daemon sets it after both `session/new` and `session/load`. Unset leaves the account\'s default (`gemini-3.7-flash-high` on the free tier).',
      'Usage: none on the ACP surface — the prompt response is `{stopReason}` only, no `usage_update`, no `_meta` (measured); jobs carry no `usage`. The server\'s stderr log echoes the internal harness\'s `usageUpdate` frames (promptTokenCount, candidatesTokenCount, thoughtsTokenCount), an undocumented debug line this bridge does not read.',
      'Credentials live in the server\'s own home, `$GEMINI_HOME/antigravity-acp/` (`~/.gemini` by default): `settings.json` records `auth.type`, the token is in the login keychain (service `gemini`, account `antigravity-acp`), conversations are SQLite files beside them. The free tier\'s Interactions may be reviewed by Google staff unless the account opts out in Antigravity\'s own settings; the ACP server exposes no such switch.',
      'The tool sandbox is derived from the session cwd (symlinks resolved), so every send\'s `cwd` must be the real project directory. No `/fleet` equivalent and no rubber-duck wrapper: output is relay-only.',
    ],
    acp: {
      clientName: 'agent-companion',
      // The registry entry's own launch args: none on macOS, `--uid=` on linux.
      args: () => (process.platform === 'linux' ? ['--uid='] : []),
      env: () => ({}),
      rotate: [],
      defaultModel: () => null,
      loadSession: true,
      permission: (env) => resolveAntigravityPermission(env),
      usage: () => null,
      // Measured 2026-09-11 on 1.1.1 (the last is the `session/load` replay).
      updates: [
        'agent_thought_chunk', 'agent_message_chunk', 'tool_call', 'tool_call_update',
        'available_commands_update', 'user_message_chunk',
      ],
      rubberDuck: false,
      // The model is per session, not per spawn: the request the daemon sends
      // after `session/new` and again after `session/load` when a model is pinned.
      setModel: ({ sessionId, model }) => ['session/set_config_option', { sessionId, configId: 'model', value: model }],
    },
    binaryEnv: 'ANTIGRAVITY_ACP_BIN',
    binaryNames: [antigravityAcpPaths().binary],
    // Prints the build stamp ("Built on …"), not a semver; the registry
    // version is the directory `current` points at.
    versionArgs: ['--version'],
    install: {
      docs: 'https://antigravity.google/docs/ide/extensions',
      commands: [
        'node scripts/install-antigravity-acp.mjs          # from the ACP registry, signature-checked on macOS',
      ],
    },
    auth: {
      docs: 'https://antigravity.google/docs/ide/extensions/zed',
      checkCommands: [],
      probe: probeAntigravityAuth,
      nextSteps: [
        'Run `node scripts/install-antigravity-acp.mjs --login` and sign in with your Google account in the browser it opens (any Antigravity plan, including the free tier).',
        'Or set `auth.type` to `gemini-api-key` in ~/.gemini/antigravity-acp/settings.json and export GEMINI_API_KEY.',
      ],
    },
    permission: {
      docs: 'https://antigravity.google/docs/ide/extensions',
      // The policy the daemon answers permission requests with. `all` is the
      // bridge's unattended default, as Copilot's --allow-all-* flags are; the
      // narrower policies never hang a run — a rejected tool tells the model.
      safeDefault: 'all',
      bridgeEnv: 'AGENT_COMPANION_ANTIGRAVITY_PERMISSION',
      modes: ANTIGRAVITY_PERMISSION_POLICIES,
      dangerousModes: [],
    },
    smoke: {
      safeByDefault: false,
      reason: 'Starts a real Antigravity turn and consumes the account\'s daily request quota.',
    },
  },
};
