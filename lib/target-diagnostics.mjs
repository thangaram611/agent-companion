...
// Codex's sandbox knob is a closed set of named modes (unlike OpenCode's
// binary prompt/skip), and TWO of them are independently dangerous
// (danger-full-access, bypass) — so this path is keyed off
// `permission.dangerousModes` (array) rather than the single `dangerousMode`
// string the OpenCode-shaped branch below uses.
const CODEX_SANDBOX_MODES = ['read-only', 'workspace-write', 'danger-full-access', 'bypass'];

function probeSandboxPermission(perm, env) {
  const raw = String(env[perm.bridgeEnv] || '').trim().toLowerCase();
  const recognized = CODEX_SANDBOX_MODES.includes(raw);
