...
// absent or corrupt. One degenerate __default__ profile mirrors the legacy
// bare-target semantics exactly (companion = default-target, copilot model from
// default-model, opencode model from env). companion may be null when no
// default-target is configured — resolveRouting turns that into
// TARGET_UNCONFIGURED.
