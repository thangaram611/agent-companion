...
// serve` today, the codex app-server broker next) are singletons shared by every
// bridge on the machine that resolves to the same host home
...
// because the thing they protect outlives any one process: the runtime is shared by
...
// The single definition of "is this pid alive" in the shipped tree — the lease
// pruner below, the hydrate ownership guard (server.mjs) and the broker's stale
// start-lock check all import THIS one. None of them may grow a second copy: a
// naive `process.kill(pid, 0)` wrapper reports a pid owned by another user as
// dead, which would make hydrate declare a live companion child orphaned on any
// multi-user box, and it accepts a pid <= 0, where `kill` addresses a process
// GROUP and answers "alive" for a torn registry or lock file.
//
// `probes/codex-app-server/stdio-lib.mjs` keeps its own one-liner. Those
// harnesses are frozen evidence of what was measured, standalone by design, and
...
export function createSharedRuntimeRegistry({ registryPath, key, identity, dispose } = {}) {
  ...
  // short-circuit, but it is not sufficient on its own: the runtime is shared by
  // every bridge process resolving to this registry while the bridge itself is
  // spawned per subagent, so a second bridge
