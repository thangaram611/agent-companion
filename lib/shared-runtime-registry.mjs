...
export function createSharedRuntimeRegistry({ registryPath, key, identity, dispose } = {}) {
  ...
  function claimDisposal(entry, { now = Date.now(), pid = process.pid } = {}) {
    ...
    if (!confirm || identity(confirm) !== identity(entry) || claim?.pid !== pid) return false;
    // Someone adopted the runtime between our decision and our claim. Stand down
    // — and withdraw the claim, or it would block adoption until the TTL expires
    // and cost them a redundant spawn for nothing.
  ...
  async function reapIdle({ idleMs, hasLiveJobs = false, now = Date.now(), pid = process.pid } = {}) {
    ...
    try { await dispose(entry); } catch { /* the runtime may already be gone */ }
    ...
    const after = read();
    if (!after || identity(after) === identity(entry)) forget();
    return true;
  }
