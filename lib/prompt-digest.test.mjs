...
test('renderDigest assembles the full digest contract from a rich prompt transcript', async () => {
  await withTempDirs(async ({ promptDir }) => {
    ...
    assert.doesNotMatch(md, /\*\*Usage:\*\*/, 'no usage reported, no usage line');
    const withUsage = buildDigest(promptId, {
      jobId: 'job-rich', status: 'completed',
      usage: { source: 'copilot-otel', input_tokens: 24869, output_tokens: 4, cached_input_tokens: null, cache_write_input_tokens: 24867, reasoning_output_tokens: null, total_tokens: 24873, model: 'claude-sonnet-5', cost: 1, cost_unit: 'copilot_premium_requests' },
    });
    assert.match(withUsage, /\*\*Usage:\*\* in=24869 out=4 cache_write=24867 total=24873 model=claude-sonnet-5 cost=1 copilot_premium_requests \(copilot-otel\)/);
