...
All six drive `bridge-server/server.mjs` over MCP stdio as a real client would; five dispatch
real Codex jobs and `acp-antigravity.mjs` dispatches real Antigravity jobs. The repo root is derived from the script location; override with
...
| `acp-antigravity.mjs` | 24 checks that the **generic ACP transport** carries Google's Antigravity ACP server end to end (docs/MVP_TRACKER.md item 8): a `target: "antigravity"` send through the real bridge and the real detached `acp-daemon --companion antigravity` completes with the companion's answer and **no** `usage` key (the ACP surface carries none), the digest is headed by the companion, the daemon owns its own socket/log/prompt stream and is recorded in `acp-daemons.json`; round two on the same thread rides the same ACP session and remembers round one; `agent_reply` is accepted as cancel + re-prompt (the hint says so) and the follow-up wins; `agent_cancel` settles `cancelled`; bridge A is SIGKILLed mid-turn, the daemon outlives it, bridge B hydrates the job as running on the same prompt id and it completes with no re-prompt; a model pin sent to the daemon lands as `session/set_config_option` in the daemon log. Spends about six Antigravity turns. |
...
node probes/smoke/acp-antigravity.mjs   # expect 24/24  (Antigravity through the ACP daemon; needs the server signed in)
```
